...
def gen1(url,student_class):
    ...
    #print(student_data)
    # Load the known face and encodingss
...
def photoUpload(b64_string,student_class):
    ...
    # nparr = np.fromstring(encoded_data.decode('base64'), np.uint8) #edited*
...
def capture_photo_py(url):
    ...
    for each in y:
        ...
        # time.sleep(0.1)
def gen(url):
    video = cv2.VideoCapture(0+url)#edited***
    time.sleep(2.0)
    global camera_status
...
def submit_live_attendance(stu_id, student_class,date):
    attendance_class={
        "forth_semester" : "INSERT INTO forth_semester(student_id,attendance_date) VALUES(?, ?);",
        "third_semester"  : "INSERT INTO third_semester(student_id,attendance_date) VALUES(?, ?);",
        "second_semester"  : "INSERT INTO second_semester(student_id,attendance_date) VALUES(?, ?);",
        "first_semester"  : "INSERT INTO first_semester(student_id,attendance_date) VALUES(?, ?);",
    }
...
def fetchAttendance(attendanceClass, attendanceDate):
    student_class={
        'first_semester'  : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM first_semester ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
        'second_semester' : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM second_semester ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
        'third_semester' : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM third_semester ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
        'forth_semester' : "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM forth_semester ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
    }
    conn = create_connection()
    cursor = conn.cursor()
    val = [attendanceDate]
...
def fetch_graph_data(graphClass):
    student_class = {
        'first_semester'  : "SELECT DISTINCT(attendance_date) FROM first_semester ORDER BY attendance_date ASC LIMIT 06 ",
        'second_semester' : "SELECT DISTINCT(attendance_date) FROM second_semester ORDER BY attendance_date ASC LIMIT 06 ",
        'third_semester' : "SELECT DISTINCT(attendance_date) FROM third_semester ORDER BY attendance_date ASC LIMIT 06 ",
        'forth_semester' : "SELECT DISTINCT(attendance_date) FROM forth_semester ORDER BY attendance_date ASC LIMIT 06 ",
    }
    attendance_class = {
        'first_semester'  : "SELECT COUNT(DISTINCT(student_id)) FROM first_semester WHERE attendance_date = ? ;",
        'second_semester' : "SELECT COUNT(DISTINCT(student_id)) FROM second_semester WHERE attendance_date = ? ;",
        'third_semester' : "SELECT COUNT(DISTINCT(student_id)) FROM third_semester WHERE attendance_date = ? ;",
        'forth_semester' : "SELECT COUNT(DISTINCT(student_id)) FROM forth_semester WHERE attendance_date = ? ;",
    }
